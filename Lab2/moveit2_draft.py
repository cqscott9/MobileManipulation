...
class MoveMe(HelloNode):
    def __init__(self):
        ...
        for i in range(4): # edited
            ...
            if i == 0:
                lift = 0.5
                arm = [self.get_joint_pos('joint_arm_l3'), self.get_joint_pos('joint_arm_l2'), self.get_joint_pos('joint_arm_l1'), self.get_joint_pos('joint_arm_l0')]
                wrist = [self.get_joint_pos('joint_wrist_yaw'), self.get_joint_pos('joint_wrist_pitch'), self.get_joint_pos('joint_wrist_roll')]
            
            if i == 1:
                lift = self.get_joint_pos('joint_lift')
                arm = [0.4/4.0, 0.4/4.0, 0.4/4.0, 0.4/4.0] # move arm forward by 0.4 meters
                wrist = [self.get_joint_pos('joint_wrist_yaw'), self.get_joint_pos('joint_wrist_pitch'), self.get_joint_pos('joint_wrist_roll')]
